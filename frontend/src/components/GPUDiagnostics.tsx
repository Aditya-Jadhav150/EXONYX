...
export default function GPUDiagnostics() {
  ...
  useEffect(() => {
    ...
    const fetchDiag = async () => {
      try {
        const res = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000') + '/api/v1/gpu/diagnostics');
        if (res.ok) {
        ...
        // Hide silently if backend is down
        setDiag(null);
      }
    };
  ...
  if (!diag) return null; // Completely hide from public if backend is not running

  const usedVRAM = diag.total_vram_mb - diag.free_vram_mb;
  const isTensorRT = diag.tensorrt_enabled;
    ...
    <div className="fixed bottom-4 left-4 z-50 font-mono text-[10px] uppercase tracking-wider">
      {!isOpen && (
        <button 
          ...
          className="bg-slate-900/80 text-slate-400 border border-slate-700 p-2 rounded-full hover:text-cyan-400 hover:border-cyan-500 transition-colors backdrop-blur-md shadow-[0_0_15px_rgba(34,211,238,0.1)]"
